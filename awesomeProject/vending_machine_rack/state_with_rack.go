...
// if Inventory is being used to store products, IGNORE RACK
import "fmt"

type State interface {
	InsertMoney(amount float64)
	SelectRack(rackId int)
	ReturnChange()
	DispenseProduct()
}
...
func (s *MoneyInsertedState) SelectRack(rackId int) {
...
func (s *ProductSelectedState) SelectRack(rackId int) {
	if !s.vendingMachine.rackMap[rackId].IsProductAvailable() {
		fmt.Println("Product not available in rack.")
		s.vendingMachine.UpdateState(s.vendingMachine.moneyInsertedState)
		return
	}
	product := s.vendingMachine.rackMap[rackId].Product
	if product.Price > s.vendingMachine.insertedMoney {
		...
		s.vendingMachine.selectedRack = s.vendingMachine.rackMap[rackId]
		fmt.Printf("rack %d selected, product selected %s.\n", s.vendingMachine.selectedRack.ID, product.Name)
...
func (s *ProductDispensedState) SelectRack(rackId int) {
	fmt.Println("rack already selected.")
}
...
func (s *ProductDispensedState) DispenseProduct() {
	product, err := s.vendingMachine.selectedRack.TransactProduct()
	if err != nil {
